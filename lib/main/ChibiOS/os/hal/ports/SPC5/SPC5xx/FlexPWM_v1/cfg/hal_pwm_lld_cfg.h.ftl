...
[#list conf.instance.flexpwm_settings.pwm0_configurations.configs.pwm_configuration_settings as settings]
  ...
  [#if ch6_callback != ""]
    ...
    [/#if]
  [/#if]
  [#assign ch7_callback = settings.channel_7_settings.channel_callback.value[0]?string?trim /]
  [#if ch7_callback != ""]
    [#if !channel_callbacks?seq_contains(ch7_callback)]
      [#assign channel_callbacks = channel_callbacks + [ch7_callback]]
    [/#if]
  [/#if]
  [/#if]
[/#list]

...
extern "C" {
    ...
    serialPortConfig_t *mockSerialPortConfig;
    serialPort_t *mockSerialPort = {

    };
    serialPortConfig_t *findSerialPortConfig(serialPortFunction_e) {
        return mockSerialPortConfig;
    }
...
TEST(GhostTest, TestCrsfFrameStatus)
{
    /* ghstFrame.frame.deviceAddress = GHST_ADDRESS_GHST_RECEIVER; */
    /* ghstFrame.frame.frameLength = 0; */
    ghstFrame.frame.type = 0;
    memset(ghstFrame.frame.payload, 0, 14);
    const uint8_t crc = ghstFrameCRC(&ghstFrame);

    EXPECT_EQ(0, crc);
    /* ghstFrame.frame.payload[GHST_FRAME_RC_CHANNELS_PAYLOAD_SIZE] = crc; */

    /* const uint8_t status = ghstFrameStatus(); */
    /* EXPECT_EQ(RX_FRAME_COMPLETE, status); */
    /* EXPECT_EQ(false, ghstFrameDone); */

    /* EXPECT_EQ(GHST_ADDRESS_GHST_RECEIVER, ghstFrame.frame.deviceAddress); */
    /* EXPECT_EQ(GHST_FRAMETYPE_RC_CHANNELS_PACKED, ghstFrame.frame.type); */
    /* for (int ii = 0; ii < GHST_MAX_CHANNEL; ++ii) { */
    /*     EXPECT_EQ(0, ghstChannelData[ii]); */
    /* } */
}
...
/*TEST(GhostTest, TestCrsfFrameStatusUnpacking)*/
/*{*/
/*    crsfFrameDone = true;*/

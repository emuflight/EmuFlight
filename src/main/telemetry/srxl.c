...
bool srxlFrameRpm(sbuf_t *dst, timeUs_t currentTimeUs) {
    ...
    if (telemetryConfig()->report_cell_voltage) {
        sbufWriteU16BigEndian(dst, getBatteryAverageCellVoltage() * 10); // Cell voltage is in units of 0.01V
    } else {
        sbufWriteU16BigEndian(dst, getBatteryVoltage() * 10);   // vbat is in units of 0.01V
    }

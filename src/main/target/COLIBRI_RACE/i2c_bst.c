...
static bool bstSlaveProcessFeedbackCommand(uint8_t bstRequest)
{
    ...
    switch (bstRequest) {
        ...
        case BST_RC_TUNING:
            ...
            bstWrite8(currentControlRateProfile->dynThrP);
            bstWrite8(currentControlRateProfile->dynThrI);
    ...
    switch (bstWriteCommand) {
        ...
        case BST_SET_RC_TUNING:
            if (bstReadDataSize() >= 10) {
                ...
                rate = bstRead8();
                currentControlRateProfile->dynThrP = MIN(rate, CONTROL_RATE_CONFIG_TPA_MAX);
                currentControlRateProfile->dynThrI = MIN(rate, CONTROL_RATE_CONFIG_TPA_MAX);

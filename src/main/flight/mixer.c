...
float cubicCurve(float in, float exp)
{
    float ans = in * (in * in * (1.0f - exp) + exp);
...
float motorCurve(float in)
{
#define RC_EXPO (0.4f)     // TODO: derive it in the future from the configuration file

    return cubicCurve(in, RC_EXPO);

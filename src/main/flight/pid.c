...
void resetPidProfile(pidProfile_t *pidProfile)
{
    RESET_CONFIG(pidProfile_t, pidProfile,
        ...
        .dterm_lowpass_hz = 200,    // NOTE: dynamic lpf is enabled by default so this setting is actually
                                    // overridden and the static lowpass 1 is disabled. We can't set this
                                    // value to 0 otherwise Configurator versions 10.4 and earlier will also
                                    // reset the lowpass filter type to PT1 overriding the desired BIQUAD setting.
        .dterm_lowpass2_hz = 250,   // second Dterm filters the errorRate
...
static float calculateAngleDerivative(int axis, float dps)	//combine the acc and gyro data
{
	static float prevVal[2] = {0.0f, };

	const float angle_dot   = (attitude.raw[axis] - prevVal[axis]) * 0.1f * pidFrequency;
	const float filteredVal = angle_dot * 0.15f + dps * 0.85f;
	prevVal[axis] = attitude.raw[axis];
...
//    }
    ...
    for (int axis = FD_ROLL; axis <= FD_YAW; ++axis) {
        ...
        errorRate = dtermLowpass2ApplyFn((filter_t *) &dtermLowpass2[axis], errorRate);

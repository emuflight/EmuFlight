...
void pidInitConfig(const pidProfile_t *pidProfile)
{
    ...
    emuBoost = pidProfile->errorBoost * (pidProfile->errorBoost / 1000000) * 0.003;
    emuBoostLimit = pidProfile->errorBoostLimit / 100.0f;
    emuBoostYaw = pidProfile->errorBoostYaw * (pidProfile->errorBoostYaw / 1000000) * 0.003;
...
void pidController(const pidProfile_t *pidProfile, const rollAndPitchTrims_t *angleTrim, timeUs_t currentTimeUs)
{
    ...
    for (int axis = FD_ROLL; axis <= FD_YAW; axis++)
    {
        ...
        float ITermNew = pidCoefficient[axis].Ki * itermErrorRate * dynCi * emuBoostMultiplier;
        if (ITermNew != 0.0f)
        {

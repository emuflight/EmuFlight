 ...
 */
 ...
 {
 spiSetDivisor(bus->busdev_u.spi.instance, BMI270_MAX_SPI_CLK_HZ);
 bmi270EnableSPI(bus);
 ...
 spiTransferByte(busdev_u.spi.instance, BMI270_REG_INIT_DATA, (uint8_t *)bmi270_maximum_fifo_config_file, sizeof(bmi270_maximum_fifo_config_file));
 ...
 */
 ...
 spiSequence(&gyro->bus, gyro->segments);
 }
 ...
 acc->gyro->bus.txBuf[0] = BMI270_REG_ACC_DATA_X_LSB | 0x80;

 busSegment_t segments[] = {
 {NULL, NULL, 8, true, NULL},
 {NULL, NULL, 0, true, NULL},
 };
 segments[0].txData = acc->gyro->bus.txBuf;
 segments[0].rxData = acc->gyro->bus.rxBuf;

 spiSequence(&acc->gyro->bus, &segments[0]);

 // Wait for completion
 spiWait(&acc->gyro->bus);
 ...
 uint16_t *accData = (uint16_t *)acc->gyro->bus.rxBuf;
 ...
 {
 uint16_t *gyroData = (uint16_t *)gyro->bus.rxBuf;
 switch (gyro->gyroModeSPI) {
 case GYRO_EXTI_INIT:
 {
 // Initialise the tx buffer to all 0x00
 memset(gyro->bus.txBuf, 0x00, 14);
 #ifdef USE_GYRO_EXTI
 ...
 if (spiUseDMA(&gyro->bus)) {


 gyro->bus.callbackArg = (uint32_t)gyro;
 gyro->bus.txBuf[0] = BMI270_REG_ACC_DATA_X_LSB | 0x80;
 gyro->segments[0].len = 14;
 gyro->segments[0].callback = bmi270Intcallback;
 gyro->segments[0].txData = gyro->bus.txBuf;
 gyro->segments[0].rxData = gyro->bus.rxBuf;
 ...
 gyro->bus.txBuf[0] = BMI270_REG_GYR_DATA_X_LSB | 0x80;

 busSegment_t segments[] = {
 {NULL, NULL, 8, true, NULL},
 {NULL, NULL, 0, true, NULL},
 };
 segments[0].txData = gyro->bus.txBuf;
 segments[0].rxData = gyro->bus.rxBuf;

 spiSequence(&gyro->bus, &segments[0]);
 ...
 spiTransfer(&gyro->bus.busdev_u.spi.instance, (uint8_t *)bmi270_tx_buf, bmi270_rx_buf, BUFFER_SIZE); // receive response

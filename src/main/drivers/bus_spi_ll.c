...
static LL_SPI_InitTypeDef defaultInit =
{
    ...
    .ClockPolarity = SPI_POLARITY_HIGH,
    .ClockPhase = SPI_PHASE_2EDGE,
};
...
void spiInitDevice(SPIDevice device)
{
    ...
    MODIFY_REG(spi->dev->CFG2, SPI_MASTER_KEEP_IO_STATE_ENABLE, SPI_MASTER_KEEP_IO_STATE_ENABLE);
...
#ifdef STM32H7
            MODIFY_REG(instance->CFG2, SPI_CFG2_CPOL | SPI_CFG2_CPHA, SPI_POLARITY_LOW | SPI_PHASE_1EDGE);
#else
            MODIFY_REG(instance->CR1, SPI_CR1_CPOL | SPI_CR1_CPHA, SPI_POLARITY_LOW | SPI_PHASE_1EDGE);
#endif
        }
        else {
            IOConfigGPIOAF(IOGetByTag(spi->sck), SPI_IO_AF_SCK_CFG_HIGH, spi->sckAF);
#ifdef STM32H7
            MODIFY_REG(instance->CFG2, SPI_CFG2_CPOL | SPI_CFG2_CPHA, SPI_POLARITY_HIGH | SPI_PHASE_2EDGE);
#else
            MODIFY_REG(instance->CR1, SPI_CR1_CPOL | SPI_CR1_CPHA, SPI_POLARITY_HIGH | SPI_PHASE_2EDGE);
#endif
        }

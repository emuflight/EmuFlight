...
/*
 * Compute SCLDEL, SDADEL, SCLH and SCLL for TIMINGR register according to
 * the I2C specification and STM32 reference manual.
 * Ported from Betaflight bus_i2c_timing.c.
 */
                                ...
                                uint8_t *scldel, uint8_t *sdadel, uint16_t *sclh, uint16_t *scll)
{
    ...
    uint8_t tAFmin = 50; // Analog filter delay (min), ns
    uint8_t tr = 100;    // Rise time (estimated), ns
    uint8_t tf = 10;     // Fall time (estimated), ns

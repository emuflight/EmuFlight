            ...
            if (tuneRx3(packet)) {
                if (((int16_t)bindOffset_max - (int16_t)bindOffset_min) <= 10) {
                    ...
                    for (uint8_t i = 0; i < REDPINE_HOP_CHANNELS; i++) {
                        if (rxFrSkySpiConfigMutable()->bindHopData[i] == 0) {
                            protocolState = STATE_BIND_TUNING1;  // retry
                            break;
                        }
                    }
...
static void initBindTuneRx(void)
{
    ...
    for (uint8_t i = 0; i < REDPINE_HOP_CHANNELS; i++) {
        rxFrSkySpiConfigMutable()->bindHopData[i] = 0;
    }
}
...
static void nextChannel(void)
{
    ...
    cc2500Strobe(CC2500_SIDLE);
    cc2500WriteReg(CC2500_23_FSCAL3, calData[rxFrSkySpiConfig()->bindHopData[channr]][0]);
    cc2500WriteReg(CC2500_24_FSCAL2, calData[rxFrSkySpiConfig()->bindHopData[channr]][1]);
    cc2500WriteReg(CC2500_25_FSCAL1, calData[rxFrSkySpiConfig()->bindHopData[channr]][2]);
        ...
        case STATE_DATA:
            if (cc2500getGdo()) {
                ...
                if (ccLen == REDPINE_PACKET_SIZE_W_ADDONS) {
                    ...
                    if ((packet[1] == rxFrSkySpiConfig()->bindTxId[0]) && (packet[2] == rxFrSkySpiConfig()->bindTxId[1])) {
                        if (isRedpineFast()) {

...
FAST_CODE FAST_CODE_NOINLINE void updateRcCommands(void)
{
    ...
    for (int axis = 0; axis < 3; axis++) {
      ...
      rcCommand[axis] = rateFineTuning(rcCommand[axis], axis);
    }
...
FAST_CODE float rateFineTuning(float rcCommand, int axis)
{
  static FAST_RAM_ZERO_INIT float lastRcCommandData[3];
  static FAST_RAM_ZERO_INIT float iterm[3];

  if (((currentControlRateProfile->rateFineTuning.rateSensCenter != 100) || (currentControlRateProfile->rateFineTuning.rateSensEnd != 100)) || ((currentControlRateProfile->rateFineTuning.rateWeightCenter > 0) || (currentControlRateProfile->rateFineTuning.rateWeightEnd > 0)))
  {
    ...
    pterm_centerStick = (1.0f - rcCommandPercent) * rcCommand * (currentControlRateProfile->rateFineTuning.rateSensCenter / 100.0f); // valid pterm values are between 50-150
    pterm_endStick = rcCommandPercent * rcCommand * (currentControlRateProfile->rateFineTuning.rateSensEnd / 100.0f);
    ...
    iterm_centerStick = (1.0f - rcCommandPercent) * rcCommandError * (1.0f - fabsf((100.0f - currentControlRateProfile->rateFineTuning.rateSensCenter) / 100.0f)); // valid iterm values are between 0-95
    iterm_endStick = rcCommandPercent * rcCommandError * (1.0f - fabsf((100.0f - currentControlRateProfile->rateFineTuning.rateSensEnd) / 100.0f));
    iterm[axis] += iterm_centerStick + iterm_endStick;
    rcCommand = rcCommand + iterm[axis]; // add the iterm to the rcCommand

    dterm_centerStick = (1.0f - rcCommandPercent) * (lastRcCommandData[axis] - rcCommand) * (currentControlRateProfile->rateFineTuning.rateWeightCenter / 100.0f); // valid dterm values are between 0-95
    dterm_endStick = rcCommandPercent * (lastRcCommandData[axis] - rcCommand) * (currentControlRateProfile->rateFineTuning.rateWeightEnd / 100.0f);

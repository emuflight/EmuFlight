...
void pgResetFn_gyroConfig(gyroConfig_t *gyroConfig)
{
    ...
    gyroConfig->gyro_lpf_type = FILTER_PT;
    gyroConfig->gyro_lpf_order = 1;
    gyroConfig->alpha = 0;
...
void dynLpfGyroUpdate(float cutoff[XYZ_AXIS_COUNT])
{
    ...
    if (gyro.dynLpfFilter == DYN_LPF_PT) {
        for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
            ptnFilterUpdate(&gyro.lpfFilter[axis].ptnFilterState, cutoff[axis], gyroDt);
        }
    } else if (gyro.dynLpfFilter == DYN_LPF_BUTTERWORTH) {
        for (int axis = 0; axis < XYZ_AXIS_COUNT; axis++) {
            biquadFilterLpfCascadeUpdate(&gyro.lpfFilter[axis].butterworthFilterState, cutoff[axis], gyro.targetLooptime);
        }
    }
}
